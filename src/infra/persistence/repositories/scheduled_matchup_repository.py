...
class ScheduledMatchupRepository(IScheduledMatchupRepository):
    ...
    def upsert(self, scheduled_matchup: ScheduledMatchupEntity) -> None:
        """
        Upsert a scheduled matchup.
        """

        # Insert if the scheduled matchup does not exist, otherwise update it.
        scheduled_matchups_collection.update_one(
            {"gameId": dict(scheduled_matchup)["gameId"]},
            {"$set": dict(scheduled_matchup)},
            upsert=True,
        )
    ...
    def get_scheduled_matchups(self) -> list[ScheduledMatchupEntity]:
        ...
        scheduled_matchups = []
        for scheduled_matchup in scheduled_matchups_collection.find():
